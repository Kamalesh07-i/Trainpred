...
export const TrainStatusCard: React.FC<TrainStatusCardProps> = ({ train, etaData }) => {
  ...
  return (
    <div className="glass-panel-glow p-6 bg-[var(--bg-surface)]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-[var(--border-c)]">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-[var(--bg-base)] border border-[var(--border-c)] flex items-center justify-center text-[#007AFF] shadow-inner">
            <Train className="w-7 h-7" />
          </div>
          <div>
            <div className="flex items-center gap-3">
              <span className="text-2xl font-black font-mono tracking-wide text-[var(--text-primary)]">
                {train.train_number}
              </span>
            ...
            <div className="flex items-center gap-2 text-xs text-[var(--text-secondary)] mt-1">
              <span className="font-semibold text-[var(--text-primary)]">{train.origin_station}</span>
              <ArrowRight className="w-3 h-3 text-[var(--text-secondary)]" />
              <span className="font-semibold text-[var(--text-primary)]">{train.destination_station}</span>
              <span className="text-[var(--text-secondary)]">•</span>
              <span>{t('totalDistance')}: {train.total_distance_km} km</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 my-5">
        <div className="p-4 rounded-xl bg-[var(--bg-base)] border border-[var(--border-c)]">
          <div className="flex items-center justify-between text-xs text-[var(--text-secondary)] mb-1">
            <span className="font-semibold">{t('currentSpeed')}</span>
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-xs font-semibold text-[var(--text-secondary)]">km/h</span>
          </div>
          <div className="text-[10px] text-[#30D158] flex items-center gap-1 mt-1 font-medium">
            <span className="w-1.5 h-1.5 rounded-full bg-[#30D158] animate-pulse" />
            {t('gpsTelemetryActive')}
          </div>
        </div>
        ...
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="flex items-center justify-between text-xs text-[var(--text-secondary)] mb-1">
            <span className="font-semibold">{t('delayStatus')}</span>
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-xs font-semibold text-[var(--text-secondary)]">{t('min')}</span>
          </div>
          <div className="text-[10px] font-bold mt-1 uppercase tracking-wider">
            {isCritical ? t('criticalDelay') : isDelayed ? t('moderateDelay') : t('onTimeSchedule')}
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-[#007AFF]/10 border border-[#007AFF]/40">
          <div className="flex items-center justify-between text-xs text-[#007AFF] mb-1">
            <span className="font-semibold">{t('aiNaturalRecovery')}</span>
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-xs font-semibold text-[#007AFF]">{t('min')}</span>
          </div>
          <div className="text-[10px] text-[#007AFF] flex items-center gap-1 mt-1 font-medium">
            <TrendingDown className="w-3 h-3 text-[#007AFF]" />
            {t('slackBufferAbsorption')}
          </div>
        </div>

        <div className="p-4 rounded-xl bg-[var(--bg-base)] border border-[var(--border-c)]">
          <div className="flex items-center justify-between text-xs text-[var(--text-secondary)] mb-1">
            <span className="font-semibold">{t('destinationEta')}</span>
      ...
      <div className="pt-2">
        <div className="flex items-center justify-between text-xs text-[var(--text-secondary)] mb-1.5">
          <span className="flex items-center gap-1.5">
            <span className="font-semibold text-[var(--text-primary)]">{t('currentSection')}</span>{' '}
            {train.next_station ? `${t('enRouteTo')} ${train.next_station}` : t('inTransit')}
          </span>
          <span className="font-mono font-bold text-[#007AFF]">{progressPct}% {t('complete')}</span>
        </div>
        <div className="w-full h-2.5 rounded-full bg-[var(--bg-base)] overflow-hidden p-0.5 border border-[var(--border-c)]">
          <div
